...
Cached object are available every time you want thanks to __active bindings__. 
To get them new `cachemerRef` object should be created by:

`cache <- cachemerRef$new()` 

To use `cachemeR` methods create new object

`cache <- cachemerRef$new()`

Available methods:

### `cache$lastCache` - get last cached object
